...
return r?decode_wrapper(r[1]):""}function utf8_encode(e){return url_decode(encode_wrapper(e))}function eExtend(e,t){var i=e
...
o=e.substring(0,s)
var c=o.split("?"),d=""
if(c.length>1)for(var l=c[1].split("&"),_=0;_<l.length;_++){var m=l[_].split("=")
r&&m[0]==t||(""==d?d="?":d+="&",d+=m[0]+"="+(m[1]?m[1]:""))}return""==d&&(d="?"),n?d="?"+t+"="+i+(d.length>1?"&"+d.substring(1):""):(""!==d&&"?"!=d&&(d+="&"),d+=t+"="+(i?i:"")),c[0]+d+a}function decodeUriParam(e){var t=e.split("=")
...
if(l=isDefined(r.limit)&&t.length>r.limit?t[Math.floor(Math.random()*t.length)]:t[d],l&&!inArray(l,o)){var _=config_s3_resource_url+s+l+config_default_s3_resource_ext
getJSON(_,function(e){if(isDefined(e)){var t=JSON.parse(e)
if(isObject(t)){var n=createThumbRecoNode(t,draw_reco_seq,i,r.currency)
...
var _=e.item_url
for(var m in i)i.hasOwnProperty(m)&&0!==i[m]&&""!==i[m]&&(_=addParameter(_,config_prefix_param+m,i[m]))
_=addParameter(_,config_prefix_param+"seq",t),isDefined(e.id)&&""!==e.id&&(_=addParameter(_,config_prefix_param+"tid",e.id))
var p=""
p+="<a href='"+_+"' target='_self' title='"+e.name+"'>",p+="<img src='"+e.img_url+"'/>",p+="</a>",c.innerHTML=p
var f=""
f+="<a href='"+_+"' target='_self' title='"+e.name+"'>",f+="<span class='name'>"+e.name+"</span>",f+="</a>",d.innerHTML=f
var u=""
...
return isDefined(e)&&isDefined(e.widget_id)&&(t=e.widget_id),i=isDefined(e)&&isDefined(e.item_id)&&""!==e.item_id?e.item_id:0,n=isDefined(e)&&isDefined(e.user_id)&&""!==e.user_id?e.user_id:0,{widget_id:t,user_id:n,item_id:i,session_id:getSessionID()}}function track(e){isDefined(e)&&isObject(e)&&isDefined(e.action)&&(e=appendPredictryData(e),isDefined(e.action.name)&&("view"===e.action.name&&1===e.items.length?(isDefined(tracking_params.algo)&&""!==tracking_params.algo&&isDefined(e.action)?(e.action.rec=!0,e.action.ori=tracking_params):isDefined(tracking_params.algo)&&""!==tracking_params.algo&&!isDefined(e.action)&&(e.action={rec:!0,ori:tracking_params}),trackView(e)):"delete_item"==e.action.name?sendImage(e):"add_to_cart"===e.action.name?(isDefined(tracking_params.algo)&&""!==tracking_params.algo&&isDefined(e.action)?(e.action.rec=!0,e.action.ori=tracking_params):isDefined(tracking_params.algo)&&""!==tracking_params.algo&&!isDefined(e.action)&&(e.action={rec:!0,ori:tracking_params}),trackAddToCart(e)):"started_checkout"===e.action.name||"started_payment"===e.action.name?trackBulk(e):"buy"===e.action.name&&e.items.length>=1?(trackBuy(e),deleteCookies()):sendImage(e)))}function setItemIntoViewSession(item_id){var viewSession=eval("("+getCookie(getCookieName("view"))+")")
...
setCookie(getCookieName("view"),value,config_tracking_session_cookie_timeout,config_cookie_path)}function lengthInUtf8Bytes(e){var t=encodeURIComponent(e).match(/%[89ABab]/g)
return e.length+(t?t.length:0)}function setRecentlyViewedSessionCookie(e){if(!isDefined(e)||!isObject(e))var e={v:[]}
var t=[]
t.push(e)
var i=JSON.stringify(t)
return setCookie(getCookieName("recentlyViewedItems"),i,config_recently_session_cookie_timeout,config_cookie_path),e}function addItemIntoRecentlyViewedSession(data){var recentlyViewedItems=eval("("+getCookie(getCookieName("recentlyViewedItems"))+")")
if(isDefined(recentlyViewedItems)&&isObject(recentlyViewedItems)){var items=recentlyViewedItems
items.push(data)
var value=JSON.stringify(items)
setCookie(getCookieName("recentlyViewedItems"),value,config_recently_session_cookie_timeout,config_cookie_path)}else setRecentlyViewedSessionCookie(data)}function setRecentlyViewedItemIdCookie(e){if(!isDefined(e)||!isObject(e))var e={v:[]}
var t=JSON.stringify(e)
...
return inArray(id,itemIds)?!0:!1}return setRecentlyViewedItemIdCookie(id),!1}function showRecentlyViewedItems(id){var recentlyViewedItems=eval("("+getCookie(getCookieName("recentlyViewedItems"))+")")
if(predictryRecent=document.querySelectorAll(".predictryRecent")[0]){var predictryRecent=document.querySelectorAll(".predictryRecent")[0],r_head=document.createElement("h2")
...
d.className="name",d.innerHTML=e.name,ul_obj.appendChild(t),t.appendChild(n),n.appendChild(r),r.appendChild(a),a.appendChild(o),n.appendChild(s),s.appendChild(c),c.appendChild(d)}i++})}}function trackView(e){isDefined(e.action)&&isDefined(e.action.rec)&&setItemIntoViewSession(e.items[0].item_id),showRecentlyViewedItems(e.items[0].item_id),checkItemId(e.items[0].item_id)||(addItemIntoRecentlyViewedSession(e.items[0]),addRecentlyViewedItemId(e.items[0].item_id)),sendImage(e)}function recentlyItemsCallback(callback){var recentlyViewedItems=eval("("+getCookie(getCookieName("recentlyViewedItems"))+")")
callback(recentlyViewedItems)}function trackBuy(data){if(isDefined(data)&&isObject(data)){data=appendPredictryData(data)
...
track(t)}function getConfig(){"object"==typeof GlobalPreidctry?console.log("Config instance is here"):console.log(GlobalPredictry)}var tenant_id=tenant_id,api_key=api_key,response={},data={},widget_id=0,widget_instance_id=0,temp_cart_id=-1,tracking_params=null,predictry_nodes={},predictryRecent={},widgets=[],is_lookup_widget=!1,config_cookie_name_prefix="_predictry_",config_cookie_disabled=!1,config_cookie_path="/",config_default_request_method="POST",config_request_method=config_default_request_method,config_default_request_content_type="application/x-www-form-urlencoded; charset=UTF-8",config_request_content_type=config_default_request_content_type,config_api_url="https://api.predictry.com/",config_cf_trackings_url="https://d1j642hg7oh3vx.cloudfront.net/",config_s3_resource_url="https://s3-ap-southeast-1.amazonaws.com/predictry/",config_api_resources=["actions","users","items","carts","cartlogs","recommendation"],config_default_actions=["view","add_to_cart","buy","started_checkout","started_payment","check_delete_item","delete_item","custom"],config_session_cookie_timeout=63072e6,config_tracking_session_cookie_timeout=12e5,config_recently_session_cookie_timeout=2592e6,config_do_not_track=!1,config_s3_data_recommendation_path="data/tenants/{tenant}/recommendations/",config_s3_data_items_path="data/tenants/{tenant}/items/",config_s3_data_category_items_path="data/tenants/{tenant}/categories/",config_default_s3_resource_ext=".json",config_cls_prefix="pry-",config_prefix_param="p_",recent_response,recent_xhr=null,draw_reco_seq=1
