...
callback(limitedItems)}else callback(recentlyViewedItems)}function typedUniversalCallback(e,t,i){function n(e){if(isDefined(e)&&isObject(e)){var n=[];[].forEach.call(e,function(r){var a=config_s3_data_items_path.replace("{tenant}",tenant_id),o=config_s3_resource_url+a+r+config_default_s3_resource_ext
getJSON(o,function(a){if(isDefined(a)){var o=isDefined(e.length)&&""!==e.length?e.length:0,s={id:isDefined(r)?r:0,len:o,algo:isDefined(t)&&""!==t?t:""},c=JSON.parse(a)
for(var d in s)s.hasOwnProperty(d)&&0!==s[d]&&""!==s[d]&&(c.item_url=addParameter(c.item_url,config_prefix_param+d,s[d]))
c.item_url=addParameter(c.item_url,config_prefix_param+"seq",draw_reco_seq),n.push(c),n.length==e.length&&(i(n),console.log(n))}else{var l={error:"There is no item details on server for item_id: "+r+" yet."}
n.push(l),n.length==e.length&&i(n)}})})}else{var r={error:"There is no data for "+t+" yet."}
