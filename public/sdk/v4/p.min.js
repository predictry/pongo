...
callback(limitedItems)}else callback(recentlyViewedItems)}function viewedAlsoViewed(e,t,i){function n(e){if(isDefined(e)&&isObject(e)){var t=[];[].forEach.call(e,function(n){var r=config_s3_data_items_path.replace("{tenant}",tenant_id),a=config_s3_resource_url+r+n+config_default_s3_resource_ext
getJSON(a,function(n){if(isDefined(n)){var r=JSON.parse(n)
t.push(r),t.length==e.length&&i(t)}})})}}getItems(e,t,n)}function trackBuy(data){if(isDefined(data)&&isObject(data)){data=appendPredictryData(data)
