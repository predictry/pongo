...
class Filters2Controller extends BaseController
{
...
//            'is_not_set'         => 'Is Not Set',
        ...
        View::share(array("ca" => get_class(), "moduleName" => "Filters", "view" => false, 'operator_types' => $this->operator_types, 'custom_script' => $this->custom_script));
    }
    ...
    public function getCreate()
    {
        ...
        $properties = ItemMeta::whereIn("item_id", $item_ids)->limit(100)->distinct("key")->lists("key", "key");
        $types      = $this->model->filter_data_type;

        return View::make(getenv('FRONTEND_SKINS') . $this->theme . ".panels.filters.form", array(
                    "type"       => "create",
    ...
    public function getItem()
    {
        ...
        $item_ids   = Item::where("site_id", $this->active_site_id)->limit(10)->get()->lists("id");
        $properties = ItemMeta::whereIn("item_id", $item_ids)->limit(10)->distinct("key")->lists("key", "key");
        $types      = $this->model->filter_data_type;

        return Response::json(
                        array("status"   => "success",
                            "response" => View::make(getenv('FRONTEND_SKINS') . $this->theme . ".panels.filters.itemfilter", array(
                                "properties" => $properties,
                                "types"      => $types,
    ...
    public function getEdit($id)
    {
        ...
        foreach ($filter_metas as $obj) {
            ...
            $this->custom_script .= "editItemFilter({$json_obj}, {$index_item_rule});"; //js func to make add itemruleedit
            $index_item_rule+=1;
        }
    ...
    public function getItemEdit()
    {
        ...
        return Response::json(
                        array("status"   => "success",
                            "response" => View::make(getenv('FRONTEND_SKINS') . $this->theme . ".panels.filters.itemfilteredit", $output)->render()));
    }
