...
class FiltersController extends \App\Controllers\BaseController
{
...
//			'is_not_set'		 => 'Is Not Set',
	...
	{
		$this->model = new \App\Models\Filter();
		...
		else
		{
			...
			{
				$filter_properties		 = \App\Models\Filtermeta::where("filter_id", $obj->id)->get()->lists("property");
	...
	{
		$inputs	 = \Input::only("name", "property", "operator_key", "value");
		$rules	 = array(
			'name'			 => 'required',
		...
		if ($validator->passes())
		{
			...
			if ($filter->id)
			{
				...
				{
					$filter_meta			 = new \App\Models\Filtermeta();
	...
	{
		$filter = \App\Models\Filter::where("id", $id)->where("site_id", $this->active_site_id)->get()->first();

		if ($filter)
		{
			$filter_metas = \App\Models\Filtermeta::where("filter_id", $filter->id)->get()->toArray();
		}
		...
		if ($validator->passes())
		{
			...
			if ($filter->id)
			{
				...
				for ($i = 0; $i < count($inputs['filter_meta_id']); $i++)
				{
					...
					{
						$filter_meta = \App\Models\Filtermeta::find($inputs['filter_meta_id'][$i]);
						if ($filter_meta)
						{
							$filter_meta->property	 = $inputs['property'][$i];
							$filter_meta->operator	 = $inputs['operator_key'][$i];
				...
				\App\Models\Filtermeta::whereNotIn("id", $filter_meta_ids)->where("filter_id", $filter->id)->delete();
			}
