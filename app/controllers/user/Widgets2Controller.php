...
class Widgets2Controller extends BaseController
{
    ...
    public function __construct()
    {
        ...
        View::share(array("ca" => get_class(), "moduleName" => "Widget", "view" => true, "custom_script" => $this->custom_script));
    ...
    public function index()
    {
        ...
        if (!is_array($data) && !is_object($data)) {
            ...
            foreach ($data->items as $item) {
                $number_of_rulesets       = WidgetRuleSet::where("widget_id", $item->id)->count();
    ...
    public function getView($id)
    {
        ...
        return View::make(getenv('FRONTEND_SKINS') . $this->theme . ".panels.widgets.viewembedjs", array("widget" => $widget, "site" => $site, 'modalTitle' => "Recommendation JS Code"));
    }
    ...
    public function getCreate()
    {
        ...
        return View::make(getenv('FRONTEND_SKINS') . $this->theme . ".panels.widgets.form", $output);
    }
            ...
            if ($widget->id) {
                for ($i = 0; $i < count($ruleset_ids); $i++) {
                    ...
                    if ($ruleset_validator->passes()) {
                        $widget_rule_sets             = new WidgetRuleSet();
                ...
                for ($i = 0; $i < count($filter_ids); $i++) {
                    ...
                    if ($filter_validator->passes()) {
                        $widget_filter            = new WidgetFilter();
    ...
    {
        $is_exists = Widget::where("id", $id)->where("site_id", $this->active_site_id)->count();
        if ($is_exists) {
            ...
            foreach ($widget_item_rulesets as $obj) {
                ...
                $this->custom_script .= "editItemWidgetRuleset({$json_obj}, {$index_item_rule});"; //js func to make add itemruleedit
                $index_item_rule+=1;
            }
            ...
            foreach ($widget_item_filters as $obj) {
                ...
                $json_obj          = json_encode($obj);
                $this->custom_script .= "editItemWidgetFilter({$json_obj}, {$index_item_filter});"; //js func to make add itemruleedit
                $index_item_filter+=1;
            }
            ...
            return View::make(getenv('FRONTEND_SKINS') . $this->theme . ".panels.widgets.form", array(
                        "type"                      => "edit",
    ...
    public function postEdit($id)
    {
        ...
        if (isset($widget) && $widget_validator->passes()) {
            ...
            $widget_item_rules = WidgetRuleSet::where("widget_id", $id)->get()->toArray();
            ...
            if ($edit_widget_filter_ids) {
                $widget_filter_ids                  = WidgetFilter::where("widget_id", $id)->get()->lists("id");
            ...
            foreach ($result_array_diff as $i => $value) {
                if ($i < count($widget_item_ruleset_ids)) {
                    $widget_ruleset             = new WidgetRuleSet();
    ...
    public function getItemwidgetRuleset()
    {
        ...
        return Response::json(
                        array("status"   => "success",
                            "response" => View::make(getenv('FRONTEND_SKINS') . $this->theme . ".panels.widgets.itemruleset", array(
                                'ruleset_list'              => $ruleset_list,
                                'type'                      => 'create',
    ...
    public function getItemEditwidgetRuleset()
    {
        ...
        return Response::json(
                        array("status"   => "success",
                            "response" => View::make(getenv('FRONTEND_SKINS') . $this->theme . ".panels.widgets.itemrulesetedit", $output)->render()));
    }
    ...
    public function getItemEditwidgetFilter()
    {
        ...
        return Response::json(
                        array("status"   => "success",
                            "response" => View::make(getenv('FRONTEND_SKINS') . $this->theme . ".panels.widgets.itemfilteredit", $output)->render()));
    }
    ...
    public function postDelete($id)
    {
        ...
        if ($is_exists) {
            ...
            WidgetRuleSet::where("widget_id", $id)->delete();
            WidgetFilter::where("widget_id", $id)->delete();
    ...
    public function getWizard()
    {
        ...
        return View::make(getenv('FRONTEND_SKINS') . $this->theme . ".panels.widgets.wizardforms", $output);
    }

    public function postAjaxWizardWidget()
    {
        $input            = Input::only("name", "description");
        $widget_model     = new Widget();
        $widget_validator = Validator::make($input, $widget_model->rules);

        if (!$widget_validator->passes()) {
            return Response::json(
                            array("status"   => "error",
                                "response" => View::make(getenv('FRONTEND_SKINS') . $this->theme . ".panels.widgets.wizardformwidget")->withErrors($widget_validator)->render()
            ));
        }
    ...
    public function postAjaxWizardCompleteWidget()
    {
        ...
        if (count($widget_ruleset_validators) === 0) {
            ...
            $widget = Widget::find($widget_model->id);
            $site   = Site::find($this->active_site_id);
            return Response::json(
                            array("status"   => "success",
