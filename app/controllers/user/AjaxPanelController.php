...
class AjaxPanelController extends \App\Controllers\BaseController
{
    ...
    public function comparisonGraph()
    {
        ...
        $stats       = ($inputs['comparison_type'] === 'sales') ? $this->_populateSalesStatsByRangesFromCache($dt_ranges, $inputs['date_unit']) : $this->_populatePageViewStatsByRangesFromCache($dt_ranges, $inputs['date_unit']);
        $other_stats = ($inputs['comparison_type'] === 'sales') ? $this->_populatePageViewStatsByRangesFromCache($dt_ranges, $inputs['date_unit']) : $this->_populateSalesStatsByRangesFromCache($dt_ranges, $inputs['date_unit']);
...
//
    ...
    function _populatePageViewStatsByRanges($dt_ranges)
    {
        ...
        if ($view_action) {
            foreach ($dt_ranges as $range) {
                $start = $range['start'];
                $end   = $range['end'];
                array_push($stats, array('start' => $start, 'end' => $end, 'stat' => $this->_getPageViewStats($view_action->id, $view_action->name, $range['start'], $range['end'])));
            }
        }
    ...
    function _getPageViewStats($view_action_id, $view_action_name, $dt_start = null, $dt_end = null)
    {
        if (!isset($dt_start) && !isset($dt_end)) {
            $dt_start = new Carbon("today"); //today begining
            $dt_end   = new Carbon("today"); //today ending
         ...
         */
        $page_view_stats['overall']     = current($this->_populateDateRangeActionStats($dt_start, $dt_end, array($view_action_id), array($view_action_name), false, false, true, true));
        $page_view_stats['recommended'] = current($this->_populateDateRangeActionStats($dt_start, $dt_end, array($view_action_id), array($view_action_name), false, true, false, true));
        $page_view_stats['regular']     = current($this->_populateDateRangeActionStats($dt_start, $dt_end, array($view_action_id), array($view_action_name), false, false, false, true));
    ...
    function _populatePageViewStatsByRangesFromCache($dt_ranges, $date_unit)
    {
        ...
        if ($view_action) {
            foreach ($dt_ranges as $range) {
                ...
                if (!$obj)
                    array_push($stats, array('start' => $start, 'end' => $end, 'stat' => $this->_getPageViewStats($view_action->id, $view_action->name, $range['start'], $range['end']), 'str_start' => $start->toDateString(), 'str_end' => $end->toDateString()));
                else
                    array_push($stats, $obj);
            }
