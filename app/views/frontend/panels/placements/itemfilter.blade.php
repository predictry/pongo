<div class="item_filter" id="item_filter{{ $index_item_placement_filter }}">
	<div class="col-sm-4 form-group">
		...
		<?php echo Form::select("filter_id[]", $filter_list, null, array('class' => 'form-control chosen-select-item', 'id' => "itemfilter{$index_item_placement_filter}", 'data-placeholder' => "Choose Item...")); ?>
	</div>
	<div class="col-sm-3 form-group">
		...
		<div class="btn-group" data-toggle="buttons">
			...
			<label class="btn btn-default <?php echo ($activated === true) ? 'active' : ''; ?>">
				<input type="radio" name="filter_active{{ $index_item_placement_filter }}" value="activated" > Activated
			</label>
			<label class="btn btn-default <?php echo ($activated === false) ? 'active' : ''; ?>">
				<input type="radio" name="filter_active{{ $index_item_placement_filter }}" value="deactivated"> Deactivated
			</label>
			@else
			<label class="btn btn-default active">
				<input type="radio" name="filter_active{{ $index_item_placement_filter }}" value="activated" checked=""> Activated
			</label>
